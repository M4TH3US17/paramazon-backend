...
CREATE TABLE media (
    ...
    type        VARCHAR(60) CHECK(type IN ('IMAGE', 'VIDEO'))     NOT NULL,
...
CREATE TABLE "user" (
    ...
    status            VARCHAR(60) CHECK(status IN ('ACTIVE', 'INACTIVE')) NOT NULL,
...
CREATE TABLE role (
    id         BIGINT GENERATED BY DEFAULT AS IDENTITY                                         NOT NULL,
    name       VARCHAR(60) CHECK(name IN ('ROLE_ADMIN', 'ROLE_USER', 'ROLE_SINGER')) DEFAULT 'ROLE_USER' NOT NULL,
...
CREATE TABLE band (
    ...
    media_id          BIGINT                                      NULL,
    status            VARCHAR(60) CHECK(status IN ('ACTIVE', 'INACTIVE')) NOT NULL,
	total_payment     DOUBLE PRECISION         DEFAULT 0.0    NOT NULL,
...
CREATE TABLE band_singer (
	...
	payment   DOUBLE PRECISION DEFAULT 0.00  NOT NULL,
...
CREATE TABLE show (
    ...
    date    DATE                                    NOT NULL,
    status  VARCHAR(60) CHECK(status IN ('ACTIVE', 'INACTIVE')) NOT NULL,
	budget  DOUBLE PRECISION  DEFAULT 0.00          NOT NULL,
